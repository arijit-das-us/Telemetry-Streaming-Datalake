...
val EVOWithHashKeyStreamBr = EVOAfterMappingToBeamsBr.withColumn("uniqueHashKey",sha1(concat(lit("EVOSilver2"), $"unix_timestamp", $"Kymeta_metricId", $"metricProviderId", $"serialnumber"))).withColumn("TimestampYYMMSSHHMMSS", from_unixtime($"unix_timestamp","yyyy-MM-dd'T'HH:mm:ss") cast TimestampType)
...
val TranWithHashKeyStreamBr = TranAfterMappingToBeamsBr.withColumn("uniqueHashKey",sha1(concat(lit("TransecSilver2"), $"unix_timestamp", $"Kymeta_metricId", $"metricProviderId", $"serialnumber"))).withColumn("TimestampYYMMSSHHMMSS", from_unixtime($"unix_timestamp","yyyy-MM-dd'T'HH:mm:ss") cast TimestampType)
