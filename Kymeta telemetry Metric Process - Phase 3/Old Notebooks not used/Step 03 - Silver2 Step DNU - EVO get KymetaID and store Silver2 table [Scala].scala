...
val DFEvoKafka = 
spark.readStream.format("kafka").option("kafka.bootstrap.servers", kafkaBrokers).option("subscribe", TargetkafkaStep1).option("startingOffsets", "latest").option("minPartitions", "10").option("failOnDataLoss", "false").load()
//DFEvo.printSchema


// COMMAND ----------

val streamingEVOSiver1 = DFEvoKafka
.select(($"key").cast("string"), ($"value").cast("string"))
.select(get_json_object(($"key"), "$.TableName").alias("TableName"), get_json_object(($"key"), "$.timestamp").alias("timestamp"), get_json_object(($"key"), "$.uniqueId").alias("uniqueId"), get_json_object(($"value"), "$.Name").alias("Name"), get_json_object(($"value"), "$.Value").alias("Value"))
