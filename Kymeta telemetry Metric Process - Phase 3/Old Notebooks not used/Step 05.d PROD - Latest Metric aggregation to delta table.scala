...
//Seq.empty[(String,Long,Long,Long,Long,Long)].toDF("remoteId","kymetaMetricId","metricProviderId","categoryId","unixTimestamp","currentTimestamp")
//.write
//.format("delta")
//.mode("overwrite")
...
//  .withColumn("uniqueHashKey",sha1(concat(lit("latestTimestamp"),$"remoteId",$"kymetaMetricId",$"metricProviderId",$"categoryId"))).withColumn("TimestampYYMMSSHHMMSS", from_unixtime($"unixTimestamp","yyyy-MM-dd'T'HH:mm:ss") cast TimestampType)
...
streamingOutputDf
  ...
  .writeStream
  .option("checkpointLocation", basePath + "DeltaTable/_checkpoint/Latest-metric")
  .format("delta")
