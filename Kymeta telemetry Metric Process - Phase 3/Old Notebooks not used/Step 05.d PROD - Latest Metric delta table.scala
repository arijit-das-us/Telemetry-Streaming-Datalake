...
//Seq.empty[(String,Long,Long,Long,Long,Long,BigDecimal,String)].toDF("remoteId","kymetaMetricId","metricProviderId","categoryId","unixTimestamp","currentTimestamp","valueInDouble","valueInString")
//.write
//.format("delta")
//.mode("overwrite")
...
//Seq.empty[(String,Long,Long,Long,Long)].toDF("remoteId","kymetaMetricId","metricProviderId","categoryId","unixTimestamp")
//.write
//.format("delta")
//.mode("overwrite")
//.partitionBy("remoteId")
...
spark.read
  .format("delta")
  .load(basePath + "DeltaTable/Metric-gold-raw")
  .filter($"dateStamp" >= from_unixtime(unix_timestamp() - lit(86400),"yyyy-MM-dd"))
...
.select(streamingOutputDf("remoteId"),streamingOutputDf("kymetaMetricId"),streamingOutputDf("metricProviderId"),streamingOutputDf("categoryId"),streamingOutputDf("currentTimestamp"), GoldDF("unixTimestamp"),GoldDF("valueInDouble"),GoldDF("valueInString"))
...
streamingDfWithValue
  ...
  .writeStream
  .option("checkpointLocation", basePath + "DeltaTable/_checkpoint/metric-gold-latest")
  .format("delta")
