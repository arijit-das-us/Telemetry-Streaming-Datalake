...
//  .withColumn("uniqueHashKey",sha1(concat(lit("custom"),$"remoteId",$"kymetaMetricId",$"metricProviderId",$"categoryId"))).withColumn("TimestampYYMMSSHHMMSS", from_unixtime($"unixTimestamp","yyyy-MM-dd'T'HH:mm:ss") cast TimestampType)
...
val dfcustomMetricSUM = streamingInputDF.join(factCustommetrics).where(streamingInputDF("kymetaMetricId") === factCustommetrics("mappingIds") && ( factCustommetrics("mappingType") === "SUM")) 
