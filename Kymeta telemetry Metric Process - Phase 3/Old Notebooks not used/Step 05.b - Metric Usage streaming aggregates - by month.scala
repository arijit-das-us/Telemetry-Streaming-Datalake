...
.filter($"unixTimestamp" >= unix_timestamp(from_unixtime(unix_timestamp() - lit(1209600),"yyyy-MM"),"yyyy-MM"))
  .withColumn("uniqueHashKey",sha1(concat(lit("monthly"),$"remoteId",$"kymetaMetricId",$"metricProviderId",$"categoryId"))).withColumn("TimestampYYMMSSHHMMSS", from_unixtime($"unixTimestamp","yyyy-MM-dd'T'HH:mm:ss") cast TimestampType)
...
//display(aggregatesDF.orderBy("unixYearMonth"))
