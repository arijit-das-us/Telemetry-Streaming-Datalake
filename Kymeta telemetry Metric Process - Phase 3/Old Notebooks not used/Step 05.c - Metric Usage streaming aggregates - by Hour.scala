...
spark.readStream
  ...
  .filter($"unixTimestamp" >= unix_timestamp(from_unixtime(unix_timestamp() - lit(1209600),"yyyy-MM-dd HH"),"yyyy-MM-dd HH"))
  .withColumn("uniqueHashKey",sha1(concat(lit("hourly"),$"remoteId",$"kymetaMetricId",$"metricProviderId",$"categoryId"))).withColumn("TimestampYYMMSSHHMMSS", from_unixtime($"unixTimestamp","yyyy-MM-dd'T'HH:mm:ss") cast TimestampType)
...
val InputDFAggType = streamingInputDF.join(
dfMetricAggType,
    expr(""" 
...
//display(aggregatesDF.orderBy("YearMonthDayHour"))
...
//display(sql("select * from MetricGoldAggByHour order by YearMonthDayHour"))