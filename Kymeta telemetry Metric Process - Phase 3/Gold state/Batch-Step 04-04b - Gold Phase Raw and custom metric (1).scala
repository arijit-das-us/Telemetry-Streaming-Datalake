...
spark.read
  .format("delta")
  .load(basePath + "DeltaTable/Batch-silver-step2")
...
spark.readStream
  .format("delta")
  .option("startingVersion", "1760494")
...
.partitionBy("dateStamp")
//.option("mergeSchema", "true")
.option("checkpointLocation", basePath + "DeltaTable/_checkpoint/metric-gold-raw-serial-asm-91921")
...
//.option("mergeSchema", "true")
.mode("append")
.save(basePath + "DeltaTable/Metric-gold-custom-sum")
