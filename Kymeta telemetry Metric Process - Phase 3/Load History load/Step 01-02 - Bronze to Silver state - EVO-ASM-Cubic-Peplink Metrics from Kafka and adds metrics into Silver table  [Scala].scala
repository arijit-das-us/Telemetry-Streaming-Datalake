...
//  .option("startingVersion", "82655")
  ...
  .filter($"timestamp" >= lit(1612137600))
...
val EVOWithHashKeyStreamBr = EVOAfterNormalizeBr.filter($"Kymeta_metricId" === 116 )

// COMMAND ----------

println(EVOWithHashKeyStreamBr.count)

// COMMAND ----------

display(EVOWithHashKeyStreamBr)
...
.partitionBy("Datestamp")
.mode("overwrite")
.save(basePath + "DeltaTable/testHistory/EVO-Metric-silver-116")
